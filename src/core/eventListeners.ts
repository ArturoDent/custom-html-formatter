...
import { isInternalConfigUpdate } from './configUpdateGuard';
...
export function registerEventListeners( context: vscode.ExtensionContext ) {
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration( async e => {
      ...
      // Ignore changes caused by our own writes
      ...
      if (
        ...
        !e.affectsConfiguration( "customHtmlFormatter.enabled" ) &&
        !e.affectsConfiguration( "customHtmlFormatter.jsBeautifyEnabled" ) &&
        !e.affectsConfiguration( "[html]" )
      ) {
        return;
      }

      // Did the HTML override block change?
      // if ( e.affectsConfiguration( "[html]" ) ) {
      //   const config = vscode.workspace.getConfiguration( "", { languageId: "html" } );
      //   const newDefault = config.get<string>( "editor.defaultFormatter" );

      //   // Now you can react to the change
      //   console.log( "HTML default formatter changed to:", newDefault );
