...
suite( "\nCustom HTML Formatter (no rules → default formatter)", function () {
  ...
  setup( async () => {
    // Clear rules so your formatter unregisters
    await vscode.workspace.getConfiguration( "customHtmlFormatter" )
      .update( "rules", undefined, vscode.ConfigurationTarget.Workspace );

    // Force built-in formatter as default
    await vscode.workspace.getConfiguration()
      .update( "[html]", { "editor.defaultFormatter": "vscode.html-language-features" },
        vscode.ConfigurationTarget.Workspace );
  } );

  teardown( async () => {
    await vscode.workspace.getConfiguration()
      .update( "[html]", undefined, vscode.ConfigurationTarget.Workspace );
  } );
